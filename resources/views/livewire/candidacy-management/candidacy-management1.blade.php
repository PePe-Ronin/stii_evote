@section('title', 'Candidacy Management - Admin')
...
<div>
    ...
    @php
        // Admin view: no student guard here. Use null to avoid calling properties on a non-student context.
        $student = null;
    @endphp

    <h2 class="mt-10 text-lg font-medium">Candidacy Management</h2>
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="col-span-12 mt-2 flex flex-wrap items-center gap-3 sm:flex-nowrap">
            @if($activeVoting)
                    ...
                    <div class="mt-5">
                        ...
                        <div class="mt-4">
                            ...
                            @if($viewingCandidacy->grade_attachment)
                                <div class="mt-2">
                                    <a href="{{ route('attachments.candidacy-grade', $viewingCandidacy->id) }}" target="_blank" class="text-blue-600 hover:text-blue-800 underline">
                        ...
                        <tr class="transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted border-b-0">
                            ...
                            <td class="shadow-[3px_3px_5px_#0000000b] first:rounded-l-xl last:rounded-r-xl box rounded-none p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 border-y border-foreground/10 bg-background first:border-l last:border-r">
                                <div class="flex items-center justify-center">
                                    ...
                                    <!-- Approve button: active only when pending -->
                                    <button
                                        @if(!$isPending) disabled @else wire:click="confirmApprove({{ $item->id }})" @endif
                                        class="@if($isPending) mr-2 flex items-center text-blue-600 hover:text-blue-800 @else mr-2 flex items-center text-gray-400 cursor-not-allowed opacity-50 @endif">
                                    ...
                                    <!-- Reject button: active only when pending -->
                                    <button
                                        @if(!$isPending) disabled @else wire:click="confirmReject({{ $item->id }})" @endif
                                        class="@if($isPending) text-red-600 hover:text-red-800 flex items-center @else text-gray-400 cursor-not-allowed opacity-50 flex items-center @endif">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <x-menu.modal 
        ...
        <div class="text-center py-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-10 h-10 text-blue-500 mx-auto mb-3">
                <path d="M20 6L9 17l-5-5"></path>
            </svg>
            <div class="mt-2 text-sm">This will set the candidacy status to <strong>Approved</strong>. Are you sure?</div>
        </div>
        <x-slot:footer>
            <button data-tw-dismiss="modal" type="button" wire:click="cancelApprove" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 text-foreground hover:bg-foreground/5 bg-background border-foreground/20 h-10 px-4 py-2 mr-1 w-24">Cancel</button>
            <button type="button" wire:click="approveConfirmed" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-blue-600 text-white hover:bg-blue-700 h-10 px-4 py-2 w-24">Approve</button>
        </x-slot:footer>
    </x-menu.modal>
    @endif
    ...
    <x-menu.modal 
        :showButton="false" 
        modalId="reject-confirm-modal" 
        title="Reject Candidacy" 
        description="Are you sure you want to reject this candidacy application?"
        size="md"
        ...
        <div class="text-center py-2">
            ...
            <div class="mt-2 text-sm">This will set the candidacy status to <strong>Rejected</strong>. Are you sure?</div>
        </div>
        <x-slot:footer>
            <button data-tw-dismiss="modal" type="button" wire:click="cancelReject" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 text-foreground hover:bg-foreground/5 bg-background border-foreground/20 h-10 px-4 py-2 mr-1 w-24">Cancel</button>
            <button type="button" wire:click="rejectConfirmed" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-red-600 text-white hover:bg-red-700 h-10 px-4 py-2 w-24">Reject</button>
        </x-slot:footer>
    </x-menu.modal>
    @endif
