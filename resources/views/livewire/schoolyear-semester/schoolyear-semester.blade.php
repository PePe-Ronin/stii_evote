<div>
    ...
    <h2 class="mt-10 text-lg font-medium">School Year and Semester List</h2>
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            ...
            <div class="mx-auto hidden opacity-70 md:block">
                @if($schoolYears->total() > 0)
                    Showing {{ $schoolYears->firstItem() }} to {{ $schoolYears->lastItem() }} of {{ $schoolYears->total() }} entries
                    @if(!empty($search))
                        (filtered from {{ \App\Models\school_year_and_semester::count() }} total entries)
                    @endif
                @else
                    No entries found
                    @if(!empty($search))
            ...
            <div class="mt-3 w-full sm:ml-auto sm:mt-0 sm:w-auto md:ml-0">
                <div class="relative w-56">
                    <input 
                        ...
                        type="text" 
                        placeholder="Search school year, semester...">
        ...
        <x-menu.modal 
            :showButton="false" 
            modalId="add-schoolyear-modal" 
            title="Add New School Year" 
            description="Fill in the details to add new school year and semester"
            size="lg"
            :isOpen="$showAddSchoolYearModal">
            <!-- Inline message area below the modal description -->
            <div class="px-5">
                <div class="min-h-5">
                    @if($errors->any())
                        <p class="text-danger text-sm break-words whitespace-normal">{{ $errors->first() }}</p>
                    @endif
                </div>
            </div>
            ...
            <form wire:submit.prevent="createSchoolYear" class="space-y-4">
                <div class="grid gap-4 gap-y-3">
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="add-school-year">
                            School Year
                        </label>
                        <input 
                    ...
                    <div class="col-start-2 col-span-3 min-h-5">
                    </div>
                    
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="add-semester">
                            Semester
                        </label>
                        <select 
                    ...
                    <div class="col-start-2 col-span-3 min-h-5">
                        @error('semester')
                            <p class="text-danger text-xs break-words whitespace-normal">{{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm text-blue-700">
                                        <strong>Note:</strong> Only one school year can be active at a time. Setting this as active will automatically deactivate all other school years.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <x-slot:footer>
                <button data-tw-dismiss="modal" type="button" wire:click="$set('showAddSchoolYearModal', false)" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 text-foreground hover:bg-foreground/5 bg-background border-foreground/20 h-10 px-4 py-2 mr-1 w-24">
                    Cancel
                </button>
                <button type="button" wire:click="createSchoolYear" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-(--color)/20 border-(--color)/60 text-(--color) hover:bg-(--color)/5 [--color:var(--color-primary)] h-10 px-4 py-2 w-24">
                    Submit
                </button>
            </x-slot:footer>
        </x-menu.modal>
        ...
        <x-menu.modal 
            :showButton="false" 
            modalId="edit-schoolyear-modal" 
            title="Edit School Year" 
            description="Update the school year and semester details"
            size="lg"
            :isOpen="$showEditSchoolYearModal">
            
            <form wire:submit.prevent="updateSchoolYear" class="space-y-4">
                <div class="grid gap-4 gap-y-3">
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="edit-school-year">
                            School Year
                        </label>
                        <input 
                            wire:model.defer="school_year" 
                    ...
                    @error('school_year') <div class="col-start-2 col-span-3 text-danger text-xs">{{ $message }}</div> @enderror
                    
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="edit-semester">
                            Semester
                        </label>
                        <select 
                            wire:model.defer="semester" 
                    ...
                    @error('semester') <div class="col-start-2 col-span-3 text-danger text-xs">{{ $message }}</div> @enderror
            ...
            <x-slot:footer>
                <button data-tw-dismiss="modal" type="button" wire:click="$set('showEditSchoolYearModal', false)" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 text-foreground hover:bg-foreground/5 bg-background border-foreground/20 h-10 px-4 py-2 mr-1 w-24">
                    Cancel
                </button>
                <button type="button" wire:click="updateSchoolYear" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-(--color)/20 border-(--color)/60 text-(--color) hover:bg-(--color)/5 [--color:var(--color-primary)] h-10 px-4 py-2 w-24">
                    Update
                </button>
            </x-slot:footer>
        </x-menu.modal>
                    ...
                    <tbody class="[&amp;_tr:last-child]:border-0">
                        ...
                        <tr class="transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted border-b-0">
                            <td class="shadow-[3px_3px_5px_#0000000b] first:rounded-l-xl last:rounded-r-xl box rounded-none p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 border-y border-foreground/10 bg-background first:border-l last:border-r">
                                <div class="font-medium">{{ $item->school_year }}</div>
                            </td>
                            <td class="shadow-[3px_3px_5px_#0000000b] first:rounded-l-xl last:rounded-r-xl box rounded-none p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 border-y border-foreground/10 bg-background first:border-l last:border-r">
                            ...
                            <td class="shadow-[3px_3px_5px_#0000000b] first:rounded-l-xl last:rounded-r-xl box rounded-none p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 border-y border-foreground/10 bg-background first:border-l last:border-r">
                                <div class="flex items-center justify-center">
                                    <button wire:click="editSchoolYear({{ $item->id }})" class="mr-3 flex items-center text-blue-600 hover:text-blue-800">
                                    ...
                                    <button wire:click="deleteSchoolYear({{ $item->id }})" class="text-red-600 hover:text-red-800 flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-1">
                                            <path d="M3 6h18"></path>
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-8 text-gray-500">
                                No school years found. Be the first to add a school year!
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <!-- END: Data List -->
        <!-- BEGIN: Pagination -->
        <x-menu.pagination :paginator="$schoolYears" :perPageOptions="[10, 25, 35, 50]" />
        <!-- END: Pagination -->
    </div>
    <!-- Delete School Year Modal -->
    @if($showDeleteSchoolYearModal)
    <x-menu.modal 
        :showButton="false" 
        modalId="delete-schoolyear-modal" 
        title="Delete School Year" 
        description="This action cannot be undone."
        size="md"
        :isOpen="$showDeleteSchoolYearModal">
        <div class="text-center py-2">
            ...
            <div class="mt-2 text-sm">Are you sure you want to delete this school year?</div>
        </div>
        <x-slot:footer>
