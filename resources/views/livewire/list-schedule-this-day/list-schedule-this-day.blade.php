<div>
    ...
    <h2 class="mt-10 text-lg font-medium">Today's Doctor Schedule - {{ $currentDay }}, {{ $today->format('M d, Y') }}</h2>
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            <div class="flex items-center gap-2 text-sm text-gray-600">
                ...
                <span class="font-medium">Showing doctors available today</span>
            </div>
            
            <div class="mx-auto hidden opacity-70 md:block">
                @if($schedules->total() > 0)
                    {{ $schedules->total() }} doctor(s) available today
                    @if(!empty($search))
                        (filtered results)
                    @endif
                @else
                        ...
                        <tr class="transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted border-b-0">
                            <td class="shadow-[3px_3px_5px_#0000000b] first:rounded-l-xl last:rounded-r-xl box rounded-none p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 border-y border-foreground/10 bg-background first:border-l last:border-r">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="font-medium text-gray-900">{{ $item->doctor->name ?? 'Unknown Doctor' }}</div>
                                        <div class="text-sm text-gray-500">{{ $item->doctor->email ?? 'No email' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="shadow-[3px_3px_5px_#0000000b] first:rounded-l-xl last:rounded-r-xl box rounded-none p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 border-y border-foreground/10 bg-background first:border-l last:border-r">
                                <div class="text-sm text-gray-600">General Practice</div>
                                <div class="text-xs text-gray-400">Primary Care</div>
                            </td>
                            <td class="shadow-[3px_3px_5px_#0000000b] first:rounded-l-xl last:rounded-r-xl box rounded-none p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 border-y border-foreground/10 bg-background first:border-l last:border-r text-center">
                                <div class="text-sm font-medium text-gray-900">{{ $today->format('M d, Y') }}</div>
                        ...
                        <tr>
                            <td colspan="4" class="text-center py-12">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 mb-1">No doctors available today</h3>
                                    <p class="text-sm text-gray-500">There are no scheduled doctors for {{ $currentDay }}, {{ $today->format('M d, Y') }}</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
