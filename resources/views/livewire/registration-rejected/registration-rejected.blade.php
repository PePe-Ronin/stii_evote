<div>
    ...
    <h2 class="mt-10 text-lg font-medium">Rejected Student Registrations</h2>
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            
            <div class="mx-auto hidden opacity-70 md:block">
                @if($students->total() > 0)
                    Showing {{ $students->firstItem() }} to {{ $students->lastItem() }} of {{ $students->total() }} rejected registrations
                    @if(!empty($search))
                        (filtered from {{ \App\Models\students::where('status', 'rejected')->count() }} total rejected entries)
                    @endif
                @else
        ...
        <x-menu.modal 
            ...
            title="Rejected Student Registration Details" 
            description="View rejected student information and rejection reason"
            size="3xl"
