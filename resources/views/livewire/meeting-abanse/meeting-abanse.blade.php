<div>
    ...
    <h2 class="mt-10 text-lg font-medium">Meeting De Abanse Management</h2>
    <div class="mt-5 grid grid-cols-12 gap-6">
        ...
        <!-- Add New Meeting De Abanse Modal -->
        <x-menu.modal 
            :showButton="false" 
            modalId="add-meeting-modal" 
            title="Add New Meeting De Abanse" 
            description="Fill in the details to add new meeting de abanse"
            size="lg"
            :isOpen="$showAddMeetingModal">
            
            <form wire:submit.prevent="createMeeting" class="space-y-4">
                <div class="grid gap-4 gap-y-3">
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="add-meeting-name">
                            Meeting Name
                        </label>
                        <input 
                            wire:model.defer="meeting_de_abanse_name" 
                            id="add-meeting-name"
                    ...
                    @error('meeting_de_abanse_name') <div class="col-start-2 col-span-3 text-danger text-xs">{{ $message }}</div> @enderror
                    
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="add-student">
                            Student
                        </label>
                        <select 
                            wire:model.defer="selected_student_id" 
                            id="add-student"
                            class="col-span-3 w-full rounded-md border bg-background px-3 py-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/5">
                    ...
                    @error('selected_student_id') <div class="col-start-2 col-span-3 text-danger text-xs">{{ $message }}</div> @enderror
                    ...
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="add-start-datetime">
                            Start Date & Time
                        </label>
                        <input 
                    ...
                    @error('start_datetime') <div class="col-start-2 col-span-3 text-danger text-xs">{{ $message }}</div> @enderror
                    
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="add-end-datetime">
                            End Date & Time
                        </label>
                        <input 
                    ...
                    @error('end_datetime') <div class="col-start-2 col-span-3 text-danger text-xs">{{ $message }}</div> @enderror
                </div>
            </form>

            <x-slot:footer>
                <button data-tw-dismiss="modal" type="button" wire:click="$set('showAddMeetingModal', false)" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 text-foreground hover:bg-foreground/5 bg-background border-foreground/20 h-10 px-4 py-2 mr-1 w-24">
                    Cancel
                </button>
                <button type="button" wire:click="createMeeting" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-(--color)/20 border-(--color)/60 text-(--color) hover:bg-(--color)/5 [--color:var(--color-primary)] h-10 px-4 py-2 w-24">
                    Submit
                </button>
            </x-slot:footer>
        </x-menu.modal>

        <!-- Edit Meeting De Abanse Modal -->
        <x-menu.modal 
            :showButton="false" 
            modalId="edit-meeting-modal" 
            title="Edit Meeting De Abanse" 
            description="Update the meeting de abanse details"
            size="lg"
            :isOpen="$showEditMeetingModal">
            
            <form wire:submit.prevent="updateMeeting" class="space-y-4">
                <div class="grid gap-4 gap-y-3">
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="edit-meeting-name">
                            Meeting Name
                        </label>
                        <input 
                            wire:model.defer="meeting_de_abanse_name" 
                    ...
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="edit-start-datetime">
                            Start Date & Time
                        </label>
                        <input 
                            wire:model.defer="start_datetime" 
                    ...
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="edit-end-datetime">
                            End Date & Time
                        </label>
                        <input 
                            wire:model.defer="end_datetime" 
            ...
            <x-slot:footer>
                <button data-tw-dismiss="modal" type="button" wire:click="$set('showEditMeetingModal', false)" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 text-foreground hover:bg-foreground/5 bg-background border-foreground/20 h-10 px-4 py-2 mr-1 w-24">
                    Cancel
                </button>
                <button type="button" wire:click="updateMeeting" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-(--color)/20 border-(--color)/60 text-(--color) hover:bg-(--color)/5 [--color:var(--color-primary)] h-10 px-4 py-2 w-24">
                    Update
                </button>
            </x-slot:footer>
        </x-menu.modal>
                    ...
                    <tbody class="[&amp;_tr:last-child]:border-0">
                        ...
                        <tr>
                            <td colspan="6" class="text-center py-8 text-gray-500">
                                No meetings found. Be the first to add a meeting de abanse!
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
    ...
    <!-- Delete Meeting De Abanse Modal -->
    @if($showDeleteMeetingModal)
    <x-menu.modal 
        :showButton="false" 
        modalId="delete-meeting-modal" 
        title="Delete Meeting De Abanse" 
        ...
        <div class="text-center py-2">
            ...
            <div class="mt-2 text-sm">Are you sure you want to delete this meeting de abanse?</div>
        </div>
        <x-slot:footer>
            <button data-tw-dismiss="modal" type="button" wire:click="$set('showDeleteMeetingModal', false)" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 text-foreground hover:bg-foreground/5 bg-background border-foreground/20 h-10 px-4 py-2 mr-1 w-24">
                Cancel
            </button>
            <button type="button" wire:click="deleteMeeting" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-(--color)/20 border-(--color)/60 text-(--color) hover:bg-(--color)/5 [--color:var(--color-danger)] h-10 px-4 py-2 w-32">
                Delete
            </button>
        </x-slot:footer>
    </x-menu.modal>
