<div>
    ...
    <h2 class="mt-10 text-lg font-medium">Department List</h2>
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            ...
            <div class="mx-auto hidden opacity-70 md:block">
                @if($departments->total() > 0)
                    ...
                    @if(!empty($search))
                        (filtered from {{ \App\Models\department::count() }} total entries)
                    @endif
                @else
                    No entries found
                    @if(!empty($search))
        ...
        <x-menu.modal 
            ...
            description="Fill in the details to add new department"
            size="lg"
            ...
            <form wire:submit.prevent="createDepartment" class="space-y-4">
                <div class="grid gap-4 gap-y-3">
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="add-department-name">
                            Department Name
                        </label>
                        <input 
            ...
            <x-slot:footer>
                <button data-tw-dismiss="modal" type="button" wire:click="$set('showAddDepartmentModal', false)" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 text-foreground hover:bg-foreground/5 bg-background border-foreground/20 h-10 px-4 py-2 mr-1 w-24">
                    Cancel
                </button>
                <button type="button" wire:click="createDepartment" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-(--color)/20 border-(--color)/60 text-(--color) hover:bg-(--color)/5 [--color:var(--color-primary)] h-10 px-4 py-2 w-24">
                    Submit
                </button>
            </x-slot:footer>
        </x-menu.modal>
        ...
        <x-menu.modal 
            ...
            <form wire:submit.prevent="updateDepartment" class="space-y-4">
                <div class="grid gap-4 gap-y-3">
                    <div class="grid grid-cols-4 items-center gap-4">
                        <label class="text-right text-sm font-medium" for="edit-department-name">
                            Department Name
                        </label>
                        <input 
                            wire:model.defer="department_name" 
                    ...
                    @error('department_name') <div class="col-start-2 col-span-3 text-danger text-xs">{{ $message }}</div> @enderror
            ...
            <x-slot:footer>
                <button data-tw-dismiss="modal" type="button" wire:click="$set('showEditDepartmentModal', false)" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 text-foreground hover:bg-foreground/5 bg-background border-foreground/20 h-10 px-4 py-2 mr-1 w-24">
                    Cancel
                </button>
                <button type="button" wire:click="updateDepartment" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-(--color)/20 border-(--color)/60 text-(--color) hover:bg-(--color)/5 [--color:var(--color-primary)] h-10 px-4 py-2 w-24">
                    Update
                </button>
            </x-slot:footer>
        </x-menu.modal>
                ...
                <table class="w-full caption-bottom border-separate border-spacing-y-[10px] -mt-2">
                    ...
                    <tbody class="[&amp;_tr:last-child]:border-0">
                        ...
                        <tr class="transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted border-b-0">
                            ...
                            <td class="shadow-[3px_3px_5px_#0000000b] first:rounded-l-xl last:rounded-r-xl box rounded-none p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 border-y border-foreground/10 bg-background first:border-l last:border-r">
                                <div class="flex items-center justify-center">
                                    <button wire:click="editDepartment({{ $item->id }})" class="mr-3 flex items-center text-blue-600 hover:text-blue-800">
                                    ...
                                    <button wire:click="deleteDepartment({{ $item->id }})" class="text-red-600 hover:text-red-800 flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-1">
                                            <path d="M3 6h18"></path>
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-8 text-gray-500">
                                No departments found. Be the first to add a department!
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <!-- END: Data List -->
        <!-- BEGIN: Pagination -->
        <x-menu.pagination :paginator="$departments" :perPageOptions="[10, 25, 35, 50]" />
        <!-- END: Pagination -->
    </div>
    ...
    <x-menu.modal 
        ...
        <div class="text-center py-2">
            ...
            <div class="mt-2 text-sm">Are you sure you want to delete this department?</div>
        </div>
        <x-slot:footer>
